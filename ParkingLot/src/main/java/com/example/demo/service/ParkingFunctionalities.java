...
public class ParkingFunctionalities {
	...
	public String park(int regno,String color)
	{
		...
		if((parkinglotrepo.getid())!=null) {
			
			//Validation for car registration number , if already exist return error
			System.out.println(slotrepo.checkRegno(regno));
			if((slotrepo.checkRegno(regno))!=null)
			{
				
				return " Already exists a car with same registration number";
			}
